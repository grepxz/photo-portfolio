...
A nested `es:` block beside each English `heading`/`tagline` — in
`gallery.yaml` for the 11, and in the `SYNTHETIC` array for the 3. Adding a
collection then means editing one place.
...
| JSON-LD | per-page nodes | `inLanguage` added; stable `@id` on `ProfessionalService` and `Person` |

The stable `@id` is the highest-risk item. An unkeyed `ProfessionalService`
emitted on 34 pages across two languages is how one photographer becomes two
...
- **Entity duplication** via unkeyed JSON-LD across locales. Mitigated by the
  stable `@id`.
